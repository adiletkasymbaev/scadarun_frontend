...
function ReactorNode({ selected, data, id }: NodeProps) {
...
const rotation = ((data as any)?.rotation ?? 0) as 0 | 90 | 180 | 270;
  ...
  const handlers: HandleType[] = [
    // LEFT — по горизонтальной линии (y ≈ 30px → ~46%)
    {
      id: "t-left",
      type: "target",
      direction: Position.Left,
      className:
        "custom-node-handle !left-[10px] !top-[46%] ",
    },
    {
      id: "s-left",
      type: "source",
    ...
    // RIGHT — та же линия
    {
      ...
      className:
        "custom-node-handle !right-[10px] !top-[46%] ",
    },
    {
      id: "s-right",
      type: "source",
  ...
  const icon = applied ? (
    <ReactorIcon stroke={applied} fill={applied} />
  ) : (
    <ReactorIcon />
  );
