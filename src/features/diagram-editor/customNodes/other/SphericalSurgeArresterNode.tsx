...
function SphericalSurgeArresterNode({ selected, data, id }: NodeProps) {
...
const rotation = ((data as any)?.rotation ?? 0) as 0 | 90 | 180 | 270;
  ...
  const icon = applied ? (
    <SphericalSurgeArresterIcon stroke={applied} fill={applied} />
  ) : (
    <SphericalSurgeArresterIcon />
  );
...
export default SphericalSurgeArresterNode;