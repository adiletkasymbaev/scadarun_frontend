...
const MIN_WIDTH = 80; // ✅ только минимум, максимума нет
...
export default function BusNode({ id, data }: FlowNode<BusNodeData>) {
  ...
  function clampByNeighbors(
    ...
    const minPct = left ? left.xPct + gapPct : 0;
    const maxPct = right ? right.xPct - gapPct : 100;
  ...
  function startDragPort(portId: string, e: React.PointerEvent) {
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      const nextPorts = ports.map((p) =>
        p.id === portId ? { ...p, xPct: desiredPct } : p
      );
  ...
  // ✅ Ресайз СЛЕВА, ПРАВЫЙ КРАЙ фиксирован:
  // width меняем, и position.x меняем на -deltaWidth (с учетом zoom и clamp по MIN)
  function onResizePointerDown(e: React.PointerEvent) {
    ...
    const onMove = (ev: PointerEvent) => {
      // 👉 тянем вправо => clientX больше => delta положительный
      const deltaClient = ev.clientX - startClientX;

      // ✅ учитываем zoom React Flow
      const zoom = rf.getViewport().zoom || 1;
      const deltaCanvas = deltaClient / zoom;

      // ✅ только минимум, без максимума
      const nextW = Math.max(MIN_WIDTH, startW + deltaCanvas);
      ...
      {ports.map((p) => (
        <div key={p.id}>
          ...
          >
            <button
              type="button"
              className="h-1.5 w-6 mt-[14.5px] rounded-full bg-transparent border border-transparent
                        hover:bg-white/20 hover:border-white/30 cursor-ew-resize"
